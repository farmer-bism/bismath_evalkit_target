...
bism_blk_buff* bism_bbf_read_blk(bism_bbf_ring* bbf_ring, uint8_t *buff_st){
  bism_blk_buff * blk_pbuff;
  uint16_t next_read;
  
  blk_pbuff = &bbf_ring->q_buff[bbf_ring->read_num];
  *buff_st = BBF_ALL_WRITABLE;
  if(blk_pbuff->state & (BISM_BUF_WLOCK | BISM_BUF_READING)){
...
static bism_blk_buff * bism_bbf_next(bism_bbf_ring* bbf_ring){
  ...
  if(next_buf->state & BISM_BUF_IDLE){
    bbf_ring->write_num = next_num;
    bbf_ring->write_buf = next_buf;
    bism_blk_set_state(next_buf, BISM_BUF_WRITING);
    return next_buf;
  }
  return NULL;
}

int bism_bbf_write(uint8_t* w_data, uint32_t len, uint8_t *buff_st, bism_bbf_ring *bbf_ring){
